...
function AdminPage() {
    ...
    // Simulación de mensajes para la bandeja
    ...
    return html`
        <div class="container mx-auto px-4 py-16 space-y-12">
            ...
            <div class="space-y-8">
                ...
                <div class="grid lg:grid-cols-2 gap-8">
                    <div class="glass-dark p-6 rounded-2xl border border-green-400/30">
                        <h2 class="text-xl text-green-400 mb-6 font-bold flex items-center gap-2">${icons.ShoppingCart(24)} Historial de Ventas</h2>
                                ...
                                <tbody class="divide-y divide-gray-700">
                                    ${state.sales.map(sale => html`
                                        <tr class="hover:bg-white/5 transition">
                                            ...
                                            <td class="p-3"><span class="px-2 py-1 rounded text-xs bg-yellow-500/20 text-yellow-400">${sale.status}</span></td>
                    ...
                    <div class="space-y-8">
                        <div class="glass-dark p-6 rounded-2xl border border-blue-400/30">
                            <div class="flex justify-between items-center mb-6 gap-4">
                                ...
                                <div class="relative w-48">
                                    ...
                                    <input type="text" id="client-search" placeholder="Buscar cliente..." oninput="setState({clientSearchQuery: this.value})" class="w-full pl-8 pr-2 py-1 text-sm rounded-lg bg-black/40 text-white border border-blue-400/20 outline-none"/>
                                </div>
                            </div>
                                        ...
                                        ${filteredClients.map(u => html`
                                            <tr class="hover:bg-white/5 transition">
                                                ...
                                                <td class="p-3 text-right">
                                                    <button onclick="prepareEditUser(${u.id})" class="text-blue-400 transition">${icons.Edit(18)}</button>
                                                    <button onclick="deleteUser(${u.id})" class="text-red-400 transition ml-2">${icons.Trash(18)}</button>
                                                </td>
                                            </tr>
                                        `).join('')}
                                    </tbody>
                        ...
                        <div class="glass-dark p-6 rounded-2xl border border-purple-400/30">
                            <div class="flex justify-between items-center mb-6 gap-4">
                                ...
                                <div class="relative w-48">
                                    <span class="absolute left-2 top-2 text-gray-400">${icons.Search(14)}</span>
                                    <input type="text" id="admin-search" placeholder="Buscar admin..." oninput="setState({adminSearchQueryUsers: this.value})" class="w-full pl-8 pr-2 py-1 text-sm rounded-lg bg-black/40 text-white border border-purple-400/20 outline-none"/>
                                </div>
                            </div>
                            <div class="overflow-x-auto max-h-48">
                                <table class="w-full text-left text-sm">
                                    <tbody class="divide-y divide-gray-700">
                                        ${filteredAdmins.map(u => html`
                                            <tr><td class="p-3 text-white font-bold">${u.name}</td><td class="p-3 text-gray-400">${u.email}</td><td class="p-3 text-right"><button onclick="prepareEditUser(${u.id})" class="text-blue-400">${icons.Edit(18)}</button></td></tr>
                                        `).join('')}
                                    </tbody>
                                </table>
                            </div>
            ...
            <div class="glass-dark p-6 md:p-8 rounded-2xl border border-amber-400/30">
                ...
                <div class="grid lg:grid-cols-3 gap-8">
                    <div class="lg:col-span-1 glass p-6 rounded-xl border border-white/10">
                        <h4 class="text-lg text-amber-300 font-bold mb-4">✨ Agregar Nuevo Producto</h4>
                        <form id="product-form" onsubmit="saveProduct(event)" class="space-y-4">
                            <input id="prod-name" type="text" placeholder="NOMBRE" value="${productData.name}" oninput="handleProductInput('name', this.value)" class="w-full p-2 rounded bg-black/40 text-white border border-amber-400/20 outline-none" required/>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <select id="prod-gender" class="w-full p-2 rounded bg-black/40 text-white border border-amber-400/20 outline-none" onchange="handleProductInput('gender', this.value)">
                                ...
                                <select id="prod-type" class="w-full p-2 rounded bg-black/40 text-white border border-amber-400/20 outline-none" onchange="handleProductInput('type', this.value)">
                            ...
                            <button type="submit" class="w-full gradient-gold py-2 rounded font-bold uppercase">GUARDAR</button>
                        </form>
                    </div>
                    <div class="lg:col-span-2 flex flex-col h-[400px]">
                        <div class="relative mb-4">
                            <span class="absolute left-3 top-2.5 text-gray-400">${icons.Search(18)}</span>
                            <input type="text" id="prod-search" placeholder="Buscar producto por nombre..." oninput="setState({adminSearchQuery: this.value})" class="w-full pl-10 pr-4 py-2 rounded-lg bg-black/40 border border-amber-400/20 outline-none focus:border-amber-400"/>
                        </div>
                        <div class="flex-1 overflow-y-auto space-y-2 pr-2">
                            ${filteredProducts.map(p => html`
                                <div class="glass p-3 rounded-lg border border-white/5 flex items-center gap-4 group">
                                    ...
                                    <div class="flex gap-2"><button onclick='prepareEditProduct(${JSON.stringify(p)})' class="p-2 bg-blue-500/20 text-blue-400 rounded">${icons.Edit(14)}</button><button onclick="deleteProduct(${p.id})" class="p-2 bg-red-500/20 text-red-400 rounded">${icons.Trash(14)}</button></div>
                                </div>
                            `).join('')}
                        </div>
                    </div>
            ...
            <div class="glass-dark p-6 rounded-2xl border border-amber-400/30">
                <h2 class="text-xl text-amber-200 font-bold mb-6">Crear Nuevo Administrador</h2>
                <form onsubmit="saveUser(event)" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <input type="text" id="user-name" placeholder="Nombre" value="${userData.name}" oninput="handleUserInput('name', this.value)" class="p-2 rounded bg-black/40 text-white border border-amber-400/20" required/>
                    <input type="email" id="user-email" placeholder="Correo" value="${userData.email}" oninput="handleUserInput('email', this.value)" class="p-2 rounded bg-black/40 text-white border border-amber-400/20" required/>
                    <input type="password" id="user-password" placeholder="Contraseña" oninput="handleUserInput('password', this.value)" class="p-2 rounded bg-black/40 text-white border border-amber-400/20" required/>
                    <button type="submit" class="gradient-gold py-2 rounded font-bold uppercase">Crear Admin</button>
                </form>
            </div>
        </div>
    `;
