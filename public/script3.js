...
document.addEventListener("DOMContentLoaded", () => {
    paypal.Buttons({
        createOrder: async function(data, actions) {
            // Aquí calculamos el total sumando los precios del carrito actual
            // Asegúrate de que tu variable global del carrito se llame "cart"
            const totalActual = cart.reduce((suma, item) => suma + (item.price * item.quantity), 0);
            ...
            const response = await fetch("/create-paypal-order", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ totalAmount: totalActual }) 
            });
        ...
        onApprove: async function(data, actions) {
            ...
            if (errorDetail?.issue === "INSTRUMENT_DECLINED") {
                return actions.restart(); // Tarjeta rechazada, intentar de nuevo
            } else if (errorDetail) {
                ...
                updateCart(); // Actualizar el carrito en la pantalla
            }
        }
